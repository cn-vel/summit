    ...
    */
    ...
    { 
        ...
        if ( ! method_exists($this, $controller_method)) {
            $this->response(array(config_item('rest_status_field_name') => 0, config_item('rest_message_field_name') => 'Unknown method.'), 404);
        }
    ...
    /**
     * Format error data node based on custom error code from config.
     *
     * @param  int  $val     The input.
     * 
     * @return array
     */
    public function error_node($code=600)
    {
        $errors = config_item('error_code');        
        $node = array(
            'status'=>0,
            'error_code'=>$code,
            'errorDesc'=>$errors[$code] 
        );
        return $node;
    }
